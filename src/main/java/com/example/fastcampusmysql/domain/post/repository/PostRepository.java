...
public class PostRepository {
    ...
    static final private  RowMapper<Post> ROW_MAPPER = (ResultSet resultSet, int numRow) -> Post
            .builder()
    ...
    static final private RowMapper<DailyPostCount> DAILY_POST_COUNT_MAPPER = (ResultSet resultSet, int rowNum) -> new DailyPostCount(resultSet.getLong("memberId"), resultSet.getObject("createdDate", LocalDate.class), resultSet.getLong("count"));
    ...
    public Page<Post> findAllByMemberId(Long memberId, Pageable pageable) {
        ...
        // 만들어야 할 객체는 page Post
        // 페이지에 필요한것들은 .. 페이징 객체 slice 상속..
        // page & slice 인터페이스 인데 우리가 담아야할 구현체들이 필요하다

        return new PageImpl(posts, pageable, getCount(memberId));
    }

    // 모든 조회에서 잠금 획득을 하면 성능 저하가 있다.
    ...
    // 커서 키를 설정할 때 커서 키도 정렬이 되어야한다.
    // 페이지 네이션 -> 정렬 키 ..
    // 사용하는 키와, 정렬되는 키 같아야한다.
    ...
    private Post update(Post post) {
        var sql = String.format("""
                ...
                """, TABLE);
        SqlParameterSource params = new BeanPropertySqlParameterSource(post);
        var updateCount = namedParameterJdbcTemplate.update(sql, params);  // update 쳤을때, 조회된 레코드의 수
